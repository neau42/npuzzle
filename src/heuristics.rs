...
pub fn distance_estimator_manhattan(taquin: & Vec<u16>, final_state: &puzzle::FinalPuzzle) -> i32 {
	...
	for i in 1..sq {
		...
		current_pos = taquin.iter().position(|r| *r == i as u16).unwrap() as i32;
		 cmpt += (((current_pos % size) - final_pos % size).abs() + ((current_pos / size) - final_pos / size).abs()) as i32 ;
	}
	cmpt
}
...
pub fn estimate_one_euclidean(final_state_posx: i32,final_state_posy: i32, taquin: & Vec<u16>, value: u16, size: i32) -> i32 {
	let pos_current = taquin.iter().position(|r| *r == value).unwrap() as i32;

	((((pos_current % size - final_state_posx) * (pos_current % size - final_state_posx))
		+ (pos_current / size - final_state_posy) * (pos_current / size - final_state_posy)) as f64).sqrt() as i32
}
...
pub fn distance_estimator_euclidean(taquin: & Vec<u16>, final_state: &puzzle::FinalPuzzle) -> i32 {
	...
	for i in 1..sq {
		cmpt += estimate_one_euclidean((final_state.position[i as usize] as i32) % size, (final_state.position[i as usize] as i32) / size, taquin, i as u16, size);
	}
	cmpt
}

pub fn estimate_one_chebyshev(final_state_posx: i32,final_state_posy: i32, taquin: & Vec<u16>, value: u16, size: i32) -> i32 {
	let pos_current = taquin.iter().position(|r| *r == value).unwrap() as i32;
...
pub fn distance_estimator_chebyshev(taquin: & Vec<u16>, final_state: &puzzle::FinalPuzzle) -> i32 {
	...
	for i in 1..sq {
		cmpt += estimate_one_chebyshev((final_state.position[i as usize] as i32) % size, (final_state.position[i as usize] as i32) / size, taquin, i as u16, size);
	}
	cmpt
}
